...
const Header = ({ sideState, setSide }: ISidebar) => {
    ...
    const [logState, setLog] = useState<boolean>(false)
    const [showPfMenu, setMenu] = useState<boolean>(false)
    ...
    useEffect(() => {
        const menuClose = (event: Event) => {
            if (pfMenuRef.current && !pfMenuRef.current.contains(event.target as Node)) {
                setMenu(false)
            }
        }
    ...
    const signState = () => {
        if (logState) {
            return (
                <div ref={pfMenuRef} className='flex' >
                    <button className='w-6 h-6 relative' onClick={() => menuToggle()}>
                        <img src={`${auth.currentUser?.photoURL}`} alt='' className='w-full h-full rounded-full'></img>
                        {pfMenu()}
                    </button>
                </div>
            )
            ...
            return (
                <button className='h-9 flex items-center text-lmBlue border border-lmBlue gap-2 px-[11px] py-[5px]' onClick={() => login()}>
    ...
    const pfMenu = () => {
        return (
            <div className={`absolute -bottom-30 z-10 right-0 flex flex-col bg-bars ${showPfMenu ? 'visible w-52' : 'invisible w-0'}`}>
                <div className='flex items-center gap-2 p-2'>
                    <div className='w-12 h-12'>
                        <img src={`${auth.currentUser?.photoURL}`} alt='' className='h-full w-full rounded-full'></img>
                    </div>
                    <p className='text-lg font-bold'>{auth.currentUser?.displayName}</p>
                </div>
                <div className='flex gap-2 p-2 items-center hover:bg-slate-600 cursor-pointer' onClick={() => { logOut(); setMenu(false) }}>
